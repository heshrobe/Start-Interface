...
;;; Some of the joshua ask-data methods (e.g. subject) try to get
;;; both the object (i.e. the actual subject in the texp) and
;;; the name of the object.  If the object is a constant or instance
;;; there are methods that fetch the appropriate symbol.
...
;;; The name is now an interned symbol
...
(defclass texp-constant-index-interner ()
  ((instance-hash-table :accessor instance-hash-table :initform (make-hash-table :test #'equal))
   (instance-name-hash-table :accessor instance-name-hash-table :initform (make-hash-table :test #'eql))
...
(defmethod intern-instance ((interner texp-constant-index-interner) (pair list))
  (let* ((ht (instance-hash-table interner))
         (name-ht (instance-name-hash-table interner))
	 (original-pair pair)
    ...
    (unless instance
      ...
      (push instance (gethash name name-ht)))
    instance))
...
(defgeneric parse-text (text &key format server kb dialog-mode))
...
(defmethod parse-text (text &key (format :text) (server "guest") (kb "no") (dialog-mode "no"))
  (let ((encoding (ecase format
		    (:xml "xml")
		    (:text "formatted-text"))))
    (multiple-value-bind (answer code) (make-a-parse-request text :encoding encoding :server server :kb kb :dialog-mode dialog-mode)
...
;;; This is based on the SAX parser, from quicklisp.
;;; Which is a callback based system, one for each XML element
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defclass xml-ternary-expression-parser (core-parser-mixin sax:default-handler)
...
(defmethod my-start-element ((parser xml-ternary-expression-parser) (localname (eql 'reply)) attrs)
  (let* ((the-attribute (first attrs))
         (quality (when the-attribute (sax::standard-attribute-value the-attribute))))
  (setf (current-reply-quality parser) quality)
  (start-trace t "~%Starting reply ~a ~a" the-attribute quality)
...
(defmethod my-start-element ((parser xml-ternary-expression-parser) (localname (eql 'instance)) attrs)
  (start-trace t "~%Starting instance ~a" attrs)
  (let* ((the-attribute (first attrs))
         (new-instance (list 'instance nil (sax::standard-attribute-value the-attribute))))
    (push (current-element parser) (stack parser))
...
;;; in the Allegro SAX parser, CONTENT in CXML, Characters
;;; always occurs within an INSTANCE or CONSTANT for which it acculuates a string
...
;;; IN cxml this only takes a single argument without start end args as in the Allegro parser
(defmethod characters ((parser xml-ternary-expression-parser) content)
...
(defmethod get-mains ((parser core-parser-mixin))
  (let* ((main-connective (gethash '|is_main| (constant-hash-table parser)))
	 (texps (as-relation main-connective))
         (answers nil))
    ;; The reason I'm pushing on to answers, rather than
    ;; using loop's collect is that the entries are in reverse
    ;; chronological order so this reverses them.
...
(defmethod get-main ((parser core-parser-mixin)) (first (get-mains parser)))

(defmethod get-mains-and-verb ((parser core-parser-mixin) continuation)
  (loop for main in (get-mains parser)
      for  verb = (name (relation main))
      do (funcall continuation main verb)))
...
(defun vocalize-generator (generator &key vocalize? wait?)
  (push (reverse (current-parse generator)) (all-parses generator))
  (let ((string (generate-text generator)))
    (when vocalize? (vocalize-text string :vocalize? vocalize? :wait? wait?))
    (values generator string)))

(defun text-for-generator (generator)
  (multiple-value-bind (texp text) (vocalize-generator generator)
    (declare (ignore texp))
    (when (search "<P>" text) (break "~a" generator))
    text))
...
(defun vocalize-text (string &key vocalize? wait?)
  #-allegro (declare (ignore vocalize? wait?))
  #+allegro
...
(defun raw? (x) (or (stringp x) (symbolp x)))

(defmacro if-raw (thing body)
  `(if (raw? ,thing) ,body ,thing))

(defmethod intern-entity ((generator question-generator) entity-name &key (definite? t) (modifier nil) (singular? t) category quantifier owner)
  (let ((interned-entity (intern-new-instance generator entity-name)))
    ...
    (when quantifier (create-quantifier generator interned-entity quantifier))
    (when owner
      (create-possesive generator interned-entity (if-raw owner (intern-entity generator owner :definite? 'none)) :is-pp? nil))
    (when category
      (let ((has-category (intern-constant generator "has_category"))
            (category (intern-constant generator category)))
        (intern-and-add-texp generator (list interned-entity has-category category))))
    interned-entity))

(defun intern-connective (generator connective)
  (intern-entity generator connective :singular? 'none :definite? 'none))
...
(defmethod create-modification ((generator question-generator) interned-entity modifier)
  (let ((interned-modifier (intern-constant generator modifier))
	(has-property (intern-connective generator "has_property")))
    (values interned-modifier (intern-and-add-texp generator (list interned-entity has-property interned-modifier)))
    ))

(defmethod create-quantifier ((generator question-generator) interned-entity quantifier)
  (let ((interned-modifier (intern-constant generator quantifier))
	(has-quantifier (intern-connective generator "has_quantifier")))
    (values interned-modifier (intern-and-add-texp generator (list interned-entity has-quantifier interned-modifier)))
    ))

(defmethod create-possesive ((generator question-generator) (owner occurs-in-mixin) (owned occurs-in-mixin) &key (is-pp? t))
...
(defmethod intern-verb :around ((generator question-generator) (verb symbol)  &rest stuff &key &allow-other-keys)
  (apply #'intern-verb generator (string verb) stuff))
...
(defmethod add-clause-modifier ((generator question-generator) clause modifier &key position)
  (let* ((has-modifier (intern-connective generator "has_modifier"))
         (modifier (intern-constant generator modifier))
         (modifier-clause (intern-and-add-texp generator (list clause has-modifier modifier))))
    (when position
      (intern-and-add-texp generator (list modifier-clause (intern-constant generator '|has_position|) (intern-constant generator position))))
    modifier-clause))
...
(defmethod generate-relative-clause ((generator question-generator) source subject relation object)
  (let ((the-clause (intern-and-add-texp generator (list subject relation object)))
        (has-rel-clause (intern-connective generator "has_rel_clause")))
    (intern-and-add-texp generator (list source has-rel-clause the-clause))
    the-clause))
...
;;; Note: At the moment generate-text with format xml doesn't work
;;; My guess is that string-for with format xml isn't quite right.
;;; text format works, so I'm not going to bother for now.
(defmethod generate-text ((parser core-parser-mixin) &key (format :text) (server "guest") (output-format :text))
  (let ((response (make-a-generate-request (string-for parser format)
    ...
    (case output-format
      ...
      (:xml
       (multiple-value-bind (flag parser) (cxml:parse response (make-instance 'xml-generate-response-parser))
	 (declare (ignore flag))
...
(define-predicate texp-and-names (texp subject s-subject-name relation relation-name object object-name) (default-predicate-model))
...
;;; This is the most general form.  Given a texp it binds
;;; the subject, relation, and object instances as well as
;;; the subject, relation and object names.
;;; A future extension would be to fetch a texp that matches the provided
;;; instances or names.  The parser doesn't store quite enough information to do that
;;; given just names, because it doesn't have an index that maps names to instances.  It does have one for
;;; for contants.

(define-predicate-method (ask-data texp-and-names) (truth-value continuation)
  (unless (eql truth-value +true+)
    (error 'ji:model-can-only-handle-positive-queries
	   :query self
	   :model (common-lisp:type-of self)))
  (with-statement-destructured (texp subject subject-name relation relation-name object object-name) self
    (flet ((body (texp)
           (let* ((t-subject (subject texp))
                  (t-subject-name (name t-subject))
                  (t-relation (relation texp))
                  ...
                  (t-object-name (name t-object)))
             (with-unification
              (unify t-subject subject)
              (unify t-subject-name subject-name)
              (unify t-relation relation)
              (unify t-relation-name relation-name)
              (Unify t-object object)
              (Unify t-object-name object-name)
              (stack-let ((backward-support (list self +true+ '(ask-data relation-of))))
                (funcall continuation backward-support))))))
      (cond
       ((and (unbound-logic-variable-p texp)
             (not (unbound-logic-variable-p subject)))
        (loop for potential-texp in (as-subject subject)
            do (body potential-texp)))
       ((unbound-logic-variable-p texp)
        (error 'ji:model-cant-handle-query
               :query self
