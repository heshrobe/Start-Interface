;;; -*- Mode: Common-lisp; package: start -*-
...
(defun make-a-parse-request (text &key (encoding "formatted-text") (server  "guest") (kb "no") (dialog-mode "no"))
...
(defun make-an-assert-request (text &key (encoding "formatted-text") (server "guest"))
  (multiple-value-bind (response code headers uri)
      (drakma:http-request *start-server*
	:parameters `(("server" . ,server)
		 ("action" . "parse")
		 ("uuid" . "iap-project")
		 ("te" . ,encoding)
  ...
  (multiple-value-bind (response code headers uri)
       ...
       *start-server*
       :parameters `(("server" . ,server)
                     ...
                     ,@(when external-sources `(("ed" . ,external-sources)))
                     ))
    (declare (ignore headers uri))
    (values response code)
    ))
...
(defun add-verb-definition (word &key past present-participle past-participle present-3rd-singular transitive? connective
				      (server "guest"))
  (let ((definition (format nil "(verb \\\"~a\\\" ~a ~a ~a ~a ~a ~a )"
			    word past present-participle past-participle present-3rd-singular transitive? connective)))
    (add-word-definition definition :server server)))
